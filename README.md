...
Because of this performance, `HashMap` is widely used, but it is not the only implementation of `Map`.  There are two reasons:

1.  Hashing can take a lot of time, so even though `HashMap` operations are constant time, the "constant" might be big.
...
For some application, we would like the speed of a hash table, but we would also like to keep the keys in order.  It turns out to be hard to do both of those things at the same time, but the `TreeMap` implementation comes close:
...
In this lab, we'll explain how binary search trees work, and then you will use one to implement a `Map`.  Then we'll analyze the performance of a tree-backed map.
...
At each level of the tree, you only have to search one child.  For example, if you are looking for `target = 4` in the previous diagram, you would start at the root, which contains the key `8`.  Because `target` is less than `8`, you would go left.  Because `target` is greater than `3` you would go right.  Because `target` is less than `6`, you would go left.  And then you would find the key you are looking for.

In this example, it would take `4` comparisons to find the target, even though the tree contains 9 keys.  In general, the number of comparisons is proportional to the height of the tree, not the number of keys in the tree.
...
By now you might see the pattern.  If we number the levels in the tree from `1` to `h`, the level with index `i` can have up to 2<sup>i-1</sup> nodes.  And the total number of nodes in `h` levels is 2<sup>h</sup>-1.  If we have
...
Here's the beginning of the `MyTreeMap` definition:
...
The instance variables are `size`, which keeps track of the number of keys, and `root`, which is a reference to the root node in the tree.  When the tree is empty, `root` is `null`.
...
Some of the methods are easy, like `size` and `clear`:
...
`clear` appears to be constant time, but consider this: when `root` is set to `null`, the garbage collector reclaims the nodes in the tree, which takes linear time.

In the next section, you'll fill in some of the other methods:



NOTE for the end: TreeMap is only faster if log n comparisons is faster than 1 hash.
...
In the subdirectory `javacs-lab09/src/com/flatironschool/javacs` you'll find the source files for this lab:

    *  `MyTreeMap.java` contains the code from the previous section and outlines for the other methods.
...
`node` is the `Node` where we should start looking, which is initially the root of the tree.  `target` is the key we're looking for. 

If `target` is `null`, `findNode` throws an exception.  Some implementations of `Map` can handle `null` as a key, but in a binary search tree, we need to be able to compare keys, so dealing with `null` is problematic.  To keep things simple, we decided that our implementation will not allow `null` as a key.

The next lines show how we can compare `target` to a key in the tree.  From the signature of `get` and `containsKey`, the compiler considers `target` to be an `Object`.  But again, for a binary search tree, we need to be able to compare keys, so we typecast `target` to `Comparable<? super K>`, which means that it is comparable to an instace of type `K` or any superclass of `K`.  
...
But dealing with Java's type system is not the point of this exercise.  Your job is to fill in the rest of `findNode`.  If it finds a node that contains `target` as a key, it should return the node.  Otherwise it should return `null`.  When you get this working, the tests for `get` and `containsKey` should pass.

Note that your solution should only search one path through the tree, so it takes time proportional to the height of the tree.  You should not search the whole tree!

*  Your next task is to fill in `containsValue`.  To get you started, we've provides a helper method, `equals`, that compares `target` and a given key.  Note that the values in the tree (as opposed to the keys) are not necessarily comparable, so we can't use `compareTo`; we have to use the `equals` method.
...
Otherwise, it calls `putHelper`, which is a helper method that searches the tree:
...
You should finish off this method so that it adds the keys from the tree to `set` in ascending order.  Hint: you might want to write a helper method, you might want to make it recursive, and you might want to [read about in-order tree traversal](https://en.wikipedia.org/wiki/Tree_traversal#In-order).

When you are done, all tests should pass.
